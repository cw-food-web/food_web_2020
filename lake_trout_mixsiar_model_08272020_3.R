...
mix <- load_mix_data(filename="lake_trout_mix.csv", #d13C and d15N of individual lkt
                     ...
                     cont_effects=NULL)
source <- load_source_data(file="lake_trout_source2.csv", #mean and sd d13C and d15N of prey
                           source_factors=NULL, 
                           conc_dep=FALSE, 
                           data_type="means", 
                           mix)
discr <- load_discr_data(filename="lake_trout_trophic_discrimination2.csv", #Post 2002 trophic discrimination
                         mix)
...
#############################################################################
#overall population plot with invasion-specific credible intervals for bull trout
#############################################################################
stats2 <- as.data.frame(stats)
ggplot(data=df.global, aes(x=x,y=..scaled.., fill=sources, colour=sources))+ 
  ...
  labs(x="Proportion of Diet",
       ...
       colour="Prey")+
  theme_bw()+
  geom_vline(xintercept = stats2[[6]][4], linetype="dotted", size=2, color="black")+ #50% credible interval of late
  geom_vline(xintercept = stats2[[6]][5], linetype="solid", size=2, color="black")+ #50% credible interval of mid
...
#############################################################################
#overall population plot with 50% credible intervals of each prey item
#############################################################################
df.global <- data.frame(sources=rep(NA,n.draws*n.sources), x=rep(NA,n.draws*n.sources))  # create empty data frame
for(i in 1:n.sources){
  df.global$x[seq(1+n.draws*(i-1),i*n.draws)] <- as.matrix(jags.1$BUGSoutput$sims.list$p.global[,i]) # fill in the p.global[i] values
  df.global$sources[seq(1+n.draws*(i-1),i*n.draws)] <- rep(source_names[i],n.draws)}  # fill in the source names
...
ggplot(data=df.global, aes(x=x,y=..scaled.., fill=sources, colour=sources))+ 
  ...
  labs(#title="Mid",
    ...
    colour="Prey")+
  theme_bw()+
  #geom_vline(xintercept=mean(subset(df.global, df.global$sources=="Bull trout")$x), size=2, color="red")+ #BLT
  geom_vline(xintercept = quantile(subset(df.global, df.global$sources=="Bull trout")$x, prob = 0.5), linetype="solid", size=2, color="black")+ #LKT
  #geom_vline(xintercept = quantile(LKT_Mid$Overlap, prob = 0.975), linetype="dotted", size=2, color="red")+ #LKT
  #geom_vline(xintercept=mean(subset(df.global, df.global$sources=="Other fish")$x), size=2, color="blue")+ #other fish
  geom_vline(xintercept = quantile(subset(df.global, df.global$sources=="Other fish")$x, prob = 0.5), linetype="dotted", size=2, color="black")+ #BLT
  #geom_vline(xintercept = quantile(BLT_Mid$Overlap, prob = 0.975), linetype="dotted", size=2, color="blue")+ #BLT
