---
title: "R markdown to accompany submission of Predatory fish invasion induces trophic dispersion in lake food webs"
...
This R markdown accompanies the journal article titled *Predatory fish invasion induces trophic dispersion in lake food webs* (hereafter called "the article"). The purpose of this R markdown is to explain to reviewers each analysis included in the article and enhance reproducibility by providing vignettes of the analyses. Questions regarding this analysis or the R markdown should be directed to Charles Wainright at charles_wainright@fws.gov.
...
Our dataset has 46 columns and 1459 rows. Each row represents one isotope sample. There are 10 lakes and 437 fish samples in this dataset.

```{r}
length(isotope_dataset) #count of columns
...
This analysis uses R package "nicheROVER" to estimate the probability that a lake trout will exist in the isotope ellipse of bull trout or vice versa in mid- and late-invasion lakes. Run the model separately for mid-invasion lakes and late-invasion lakes.
...
data$timestep <- data$year-1969 #calculate timestep for each survey year. 1969 is earliest survey year.
tail(data)
...
Producing this:
$$
Predicted~conversion = \hat{y} = \frac{1}{1+e^{-(-1.97+0.091\cdot~x)}}
$$
...
ggplot(data = data, aes(x = year,y = conv))+
  ...
  scale_x_continuous(name = "\nSurvey year",
                     ...
                     limits = c(1954,2031),
                     sec.axis = sec_axis(trans = ~.-1969, #1969 is the first survey year
                                       name = "Timestep (years)\n",
